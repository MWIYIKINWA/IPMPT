   ...
   if(isset($_POST['reg'])){
	...
	$q_checkadmin = $conn->query("SELECT * FROM `admin_login` WHERE `admin_user` = '$user_email'") or die(mysqli_error($conn));
		...
		if($v_checkadmin == 1){
			...
			$conn->query("INSERT INTO admin_login(name,admin_user,admin_password,admin_status)VALUES('$user_name', '$user_email', '$user_password', '$user_status')") or die(mysqli_error($conn));
			echo '
				<script type = "text/javascript">
